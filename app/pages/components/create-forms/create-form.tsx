...
const CreateForm: React.FC<CreateFormProps> = ({ close, create, title, placeholder }) =>{
    ...
    return (
        <form style={formStyle} onSubmit={handleAdd}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <label style={{ fontWeight: 600, fontSize: 16, color: "#657786" }}>{title}</label>
                <span onClick={close} style={{ float: "right", cursor: "pointer", color: "#ff3246" }} title="Close">
            ...
            <div style={{ display: "flex", gap: 4, flexDirection: "row" }}>
                <input placeholder={placeholder} value={name} onChange={(e) => setName(e.target.value)} style={{ flex: 1, padding: 8, borderRadius: 6, border: "1px solid #dfe7ef" }} />
