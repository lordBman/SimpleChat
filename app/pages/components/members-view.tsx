import { Group, Member } from "@simplechat/shared/models";
import React from "react";
import { useMembersContext } from "simplechat_provider/src/contexts";
import { useAppContext } from "../providers/app-provider";
import CircleLoading from "./loading";

export interface MemberViewProps{
    member: Member
}

const MemberView: React.FC<MemberViewProps> = ({ member }) =>{
    const { refreshMembers, accept, remove } = useMembersContext();
    const { makeCurrent } = useAppContext();
    const { user } = useAppContext();

    const cancelRequest = () => remove(member.group.id);

    let loading = false;
    let requesting = !member?.accepted && member?.details.id === user?.id;
    let requested = !member?.accepted && member?.details.id === user?.id;

    const message = () => member?.accepted && makeCurrent(member);

    return (
        <div className="friends-search-result-item-container" onClick={message}>
            <div className="messages-item-profile-container">
                <div className="messages-item-profile">
                    <svg xmlns="http://www.w3.org/2000/svg" width="30px" height="30px" viewBox="0 0 24 24">
                        <g fill="none">
                            <path d="m12.594 23.258l-.012.002l-.071.035l-.02.004l-.014-.004l-.071-.036q-.016-.004-.024.006l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427q-.004-.016-.016-.018m.264-.113l-.014.002l-.184.093l-.01.01l-.003.011l.018.43l.005.012l.008.008l.201.092q.019.005.029-.008l.004-.014l-.034-.614q-.005-.019-.02-.022m-.715.002a.02.02 0 0 0-.027.006l-.006.014l-.034.614q.001.018.017.024l.015-.002l.201-.093l.01-.008l.003-.011l.018-.43l-.003-.012l-.01-.01z"/><path fill="currentColor" d="M12 12c1.873 0 3.57.62 4.815 1.487c1.183.825 2.185 2.051 2.185 3.37c0 .724-.309 1.324-.796 1.77c-.458.421-1.056.694-1.672.88C15.301 19.88 13.68 20 12 20s-3.301-.12-4.532-.493c-.616-.186-1.214-.459-1.673-.88C5.31 18.182 5 17.582 5 16.858c0-1.319 1.002-2.545 2.185-3.37C8.43 12.62 10.127 12 12 12m7 1c1.044 0 1.992.345 2.693.833c.64.447 1.307 1.19 1.307 2.096c0 .517-.225.946-.56 1.253c-.306.281-.684.446-1.029.55c-.47.142-1.025.215-1.601.247c.122-.345.19-.72.19-1.122c0-1.535-.959-2.839-2.032-3.744A4.8 4.8 0 0 1 19 13M5 13q.537.002 1.032.113C4.96 14.018 4 15.322 4 16.857c0 .402.068.777.19 1.122c-.576-.032-1.13-.105-1.601-.247c-.345-.104-.723-.269-1.03-.55A1.68 1.68 0 0 1 1 15.93c0-.905.666-1.649 1.307-2.096A4.76 4.76 0 0 1 5 13m13.5-6a2.5 2.5 0 1 1 0 5a2.5 2.5 0 0 1 0-5m-13 0a2.5 2.5 0 1 1 0 5a2.5 2.5 0 0 1 0-5M12 3a4 4 0 1 1 0 8a4 4 0 0 1 0-8"/>
                        </g>
                    </svg>
                </div>
            </div>
            <div className="friends-search-result-item-details">
                <div className="friends-search-result-item-name">{member?.group.name}</div>
                <div className="friends-search-result-item-email">{member?.group.organization?.name ?? member?.details.name}</div>
                { !loading && <div style={{ alignSelf: 'end', gap: 10, display: "flex" }}>
                    { requested && <button onClick={cancelRequest} className="friends-search-result-item-button">Cancel</button> }
                    { requesting && <button className='friends-search-result-item-button'>Decline</button> }
                </div> }
                { loading && <div style={{ alignSelf: 'end'}}>
                        <CircleLoading message="requesting" labelStyle={{ fontSize: 12, color: "grey", fontWeight: 300, letterSpacing: 1.5 }} />
                    </div>
                }
            </div>
        </div>
    );
}

export interface MemberResultViewProps{ 
    group: Group, member?: Member 
}
const MemberResultView: React.FC<MemberResultViewProps> = ({ group, member }) =>{
    const { refreshMembers, accept, remove } = useMembersContext();
    const { makeCurrent } = useAppContext();
    const { user } = useAppContext();

    const acceptRequest = () => accept(group.id);
    const cancelRequest = () => remove(group.id);

    let loading = false;
    let requesting = !member?.accepted && member?.details.id === user?.id;
    let requested = !member?.accepted && member?.details.id === user?.id;

    const message = () => member?.accepted && makeCurrent(member);

    return (
        <div className="friends-search-result-item-container" onClick={message}>
            <div className="messages-item-profile-container">
                <div className="messages-item-profile">{member?.details.name.charAt(0).toUpperCase()}</div>
            </div>
            <div className="friends-search-result-item-details">
                <div className="friends-search-result-item-name">{member?.details.name}</div>
                <div className="friends-search-result-item-email">{member?.details.email}</div>
                { !loading && <div style={{ alignSelf: 'end', gap: 10, display: "flex" }}>
                    { requesting  && <button onClick={acceptRequest} className="friends-search-result-item-button">Accept</button> }
                    { requested && <button onClick={cancelRequest} className="friends-search-result-item-button">Cancel</button> }
                    { requesting && <button className='friends-search-result-item-button'>Decline</button> }
                </div> }
                { loading && <div style={{ alignSelf: 'end'}}>
                        <CircleLoading message="requesting" labelStyle={{ fontSize: 12, color: "grey", fontWeight: 300, letterSpacing: 1.5 }} />
                    </div>
                }
            </div>
        </div>
    );
}

export { MemberView, MemberResultView }