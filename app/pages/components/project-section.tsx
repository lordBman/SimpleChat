...
const ProjectSection: React.FC<React.PropsWithChildren<ProjectSectionProps>> = ({ title, icon, add, children }) =>{
    return (
        <section style={{ marginTop: "20px" }}>
            <div style={{ display: "flex", flexDirection: "row", gap: 10, width: "100%", alignItems: "end" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "42px", height: "42px", color: "var(--primary)", border: "solid 1px", borderRadius: "50%", borderColor: "grey", background: "linear-gradient(135deg,#dbeafe,#e6f0ff)" }}>
                    { icon }
                </div>
                <div style={{  display: "flex", flex: 1, flexDirection: "row", justifyContent: "space-between", alignItems: "end", marginBottom: "21px" }}>
                    <h4 style={{ fontWeight: "lighter" }}>{title}</h4>
                    <span style={{ color: "var(--primary)", cursor: "pointer" }} onClick={add}>
            ...
            <div style={{ borderTop: "1px solid #eee", padding: 12, paddingTop: "20px", marginTop: "-21px", marginLeft: "21px" }}>
                { children }
            </div>
        </section>
    );
