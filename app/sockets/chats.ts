...
const chatSocketHandler = (ws: ElysiaWS, client: Client, operation: string, data: ChatData) =>{
    ...
    switch(operation as WSChatOperation){
        case WSChatOperation.SendMessage:
            if(data.friendID === undefined && data.groupID === undefined){
                ws.send({ path, operation: operation, message: "either friendID or groupID must be provided to send message", status: 400 });
                return;
            }else{
                chatModel.create({ ...data, message: data.message!, client}).then((chat)=>{
                    ...
                    if(error instanceof Err){
                        ...
                        ws.send({ path, operation: operation, message: err.message, status: err.code });
                    }else{
                        ws.send({ path, operation: operation, message: "an internal server error occurred when processing message", status: 503 });
                    }
                });
            }
            break;
        case WSChatOperation.ReplyMessage:
            if(data.friendID === undefined && data.groupID === undefined){
                ws.send({ path, operation: operation, message: "either friendID or groupID must be provided to reply message", status: 400 });
            }else{
        ...
        case WSChatOperation.EditMessage:
            if(data.friendID === undefined && data.groupID === undefined){
                ws.send({ path, operation: operation, message: "either friendID or groupID must be provided to edit message", status: 400 });
                return;
            }else{
        ...
        case WSChatOperation.DeleteMessage:
            if(data.friendID === undefined && data.groupID === undefined){
                ws.send({ path, operation: operation, message: "either friendID or groupID must be provided to delete message", status: 400 });
                return;
            }else{
            ...
            if(data.friendID === undefined && data.groupID === undefined){
                ...
                chatModel.seen({ ...data, chatID: data.chatID!, client}).then(()=>{
                    ...
                    if(error instanceof Err){
                        ...
                        ws.send({ path, operation: operation, message: "an internal server error occurred when processing read receipt", status: 503 });
                    }
                });
            }
            break;
        case WSChatOperation.Subscribe:
            if(data.friendID === undefined && data.groupID === undefined){
                ws.send({ path, operation: operation, message: "either friendID or groupID must be provided to subscribe", status: 400 });
                return;
            }else{
                ws.subscribe(data.friendID ?? data.groupID!);
                ws.send({ path, operation: operation, message: "subscribed successfully", status: 200 });
            }
            break;
        case WSChatOperation.Unsubscribe:
            if(data.friendID === undefined && data.groupID === undefined){
                ws.send({ path, operation: operation, message: "either friendID or groupID must be provided to unsubscribe", status: 400 });
                return;
            }else{
                ws.unsubscribe(data.friendID ?? data.groupID!);
                ws.send({ path, operation: operation, message: "unsubscribed successfully", status: 200 });
            }
            break;
        default:
            ws.send({ path, operation: operation, message: "invalid operation", status: 400 });
    }
}
