...
class UserModel{
    ...
    async get(user: User): Promise<User & { chats: Array<Channel | Group> } & { firends: Friend[] } & { members: Member[] } | undefined>{
        try{
            ...
            const firends = (await this.database.client.friend.findMany({
                where: { OR: [{ requesterID: user.id }, { acceptorID: user.id }, ] }, include:{ requester: true, acceptor: true, channel: true }
            }));

            const channels: Channel[] = firends.filter((predicate)=>{ return predicate.accepted && predicate.channel  }).map((value)=>{
            ...
            return { ...init!, chats, firends, members };
        }catch(error){
            this.database.errorHandler.add(HttpStatusCode.InternalServerError, `${error}`, "error encountered when creating user");
        }
    }
