...
const Connections = () =>{
    ...
    const [results, setResults] = useState<Array<{ user: Credential, friend?: FriendResponse } | { group: GroupResponse, member?: MemberResponse }>>([]);
    ...
    const views = useMemo(()=>{
        let responses: Array<FriendResponse | MemberResponse> = [];
        switch(filter){
            case Filter.all:
        ...
        while(index < sorted.length){
            ...
            if(name.charAt(0).toUpperCase() === letter){
                if((item as any).group){
                    const member = item as MemberResponse;
        ...
        <div className="chats-main-container">
            ...
            { query.length > 0 && <div id="friends-search-results">{results.map((result, index)=> {
                if((result as any).user){
                    const init = result as { user: Credential, friend?: FriendResponse };

                    return <FriendResultView result={init} key={index}/>
                }else{
                    const init = result as { group: GroupResponse, member?: MemberResponse };
