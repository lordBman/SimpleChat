...
const isFriend = (current: Member | Friend) => "acceptorID" in current;
const isMember = (current: Member | Friend) => "group" in current;
...
const Chat = () =>{
    const { user, current } = React.useContext(AppContext) as AppContextType;
    const { send, chats, typing } = React.useContext(ChatContext) as ChatContextType;
    ...
    const initChats = useMemo(()=>{
        ...
        if(current){
            if(isFriend(current)){
                initChats = chats[(current as Friend).id]; 
            }else if(isMember(current)){
                initChats = chats[(current as Member).group.id];
            }
        }
    ...
    const initial = useMemo(()=>{                                                                                                                                                                           
        if(current){
            if(isFriend(current)){
                const friend = (current as Friend);
                if(friend?.acceptor.id === user?.id){
                ...
                const group = (current as Member).group;
            ...
            <div className="chat-header">
                <div className="chat-header-profile">
                    ...
                    <div className="messages-item-profile-container">
                        { current && isMember(current) && <div className="messages-item-profile">
                            <span className="heroicons--user-group"></span>
                        </div> }
                        { (!current || !isMember(current)) && <div className="messages-item-profile">{initial.init}</div> }
                    </div>
